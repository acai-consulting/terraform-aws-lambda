...
| <a name="input_execution_iam_role_settings"></a> [execution\_iam\_role\_settings](#input\_execution\_iam\_role\_settings) | Settings of the for Lambda execution IAM role. | <pre>object({<br>    new_iam_role = optional(object({<br>      name                        = optional(string)<br>      path                        = optional(string, "/")<br>      permissions_boundary_arn    = optional(string)<br>      permission_policy_arn_list  = optional(list(string), [])<br>      permission_policy_json_list = optional(list(string), [])<br>    }), null)<br>    existing_iam_role_name               = optional(string, null)<br>    permissions_fully_externally_managed = optional(bool, false)<br>  })</pre> | <pre>{<br>  "new_iam_role": {<br>    "path": "/",<br>    "permission_policy_arn_list": [],<br>    "permission_policy_json_list": []<br>  },<br>  "permissions_fully_externally_managed": false<br>}</pre> | no |
...
[module-version-shield]: https://img.shields.io/badge/module_version-$SEMANTIC_RELEASE_NEXT_RELEASE_VERSION-CB224B?style=flat
