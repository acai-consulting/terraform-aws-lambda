variable "lambda_settings" {
  ...
  type = object({
    ...
    package = object({
      ...
      files_to_inject   = optional(list(string), null) # map with source - path/filename as key and target - path/filename as value
      content_to_inject = optional(map(string), null)  # map with path/filename as key and file-content as value
    })
